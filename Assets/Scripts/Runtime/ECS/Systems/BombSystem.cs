...
namespace MyGame.ECS.Bomb
{
    ...
    /// kills all enemies, and grants invincibility for the bomb duration.
    /// </summary>
    [BurstCompile]
        ...
        public void OnUpdate(ref SystemState state)
        {
                    ...
                    .WithEntityAccess())
            {
                    ...
                    && bombData.ValueRO.CooldownTimer <= 0f)
                {
                    ...
                    var enemyBulletQuery = SystemAPI.QueryBuilder()
                        .WithAll<EnemyBulletTag>()
                        .Build();
                    var enemyBulletEntities = enemyBulletQuery.ToEntityArray(Allocator.Temp);
                    for (int i = 0; i < enemyBulletEntities.Length; i++)
                    {
                        ecb.DestroyEntity(enemyBulletEntities[i]);
                    }
                    enemyBulletEntities.Dispose();
